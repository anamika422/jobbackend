...
if(password.length<7|| password.length>20){
    return res.json({msg:"password length cannot be less than 7 or more than 20",success:false})
}
    ...
    try{
        ...
        if(findUser){
            ...
            if(comparePassword){
                ...
                res.json({msg:"login successfully",success:true,token:token, role:findUser.role, tokenexpires:'24h'})
            }else{
...
// }catch(error){
    // res.status(500).json({msg:"error in update user",success:false, error:error.message})
// }
    }
}
...
const deleteUser = async(req,res)=>{
    ...
    res.json({msg:"user deleted successfully"})
        ...
        return res.json({msg:"not authorized to delete this account", success:false})
    }
}
catch(error){
    res.status(500).json({msg:"error in deleting user",success:false})
}
}
...
const resetPassword = async(req,res)=>{
       try {
        ...
        res.json({msg:"error in updating password", success:false, errror:error.message})
       }
